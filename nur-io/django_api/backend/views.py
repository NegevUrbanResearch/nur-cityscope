...
class CustomActionsViewSet(viewsets.ViewSet):
    ...
    def get_image_data(self, request):
        ...
        if indicator_param:
            ...
            if indicator_id:
                globals.INDICATOR_ID = indicator_id
                print(f"Using indicator from query parameter: {indicator_param} (ID: {indicator_id})")
        ...
        try:
            ...
            map_options = [
                ...
                f"maps/{indicator_name.replace(' ', '').replace(' ', '_').lower()}_{state_year}.html",
                # Option 2: Category-based match for current year
                f"maps/{category}_{state_year}.html",
                # Option 3: Indicator name only (any year)
                f"maps/{indicator_name.replace(' ', '').replace(' ', '_').lower()}_2023.html",
    ...
    def get_deckgl_data(self, request):
        ...
        processed_data = self._process_data_for_deckgl(indicator_data, indicator_type, year)
        
        # Return processed data with cache control headers
        response = JsonResponse(processed_data, safe=False, json_dumps_params={'default': str})
        self._add_no_cache_headers(response)
        return response
    ...
    def _process_data_for_deckgl(self, indicator_data, indicator_type, year):
        ...
        try:
            ...
            try:
                geojson = IndicatorGeojson.objects.filter(indicatorData=indicator_data).first()
                if geojson and geojson.geojson:
                    ...
                    try:
                        ...
                        if isinstance(geojson.geojson, str):
                            ...
                            raise ValueError(f"Unexpected geojson type: {type(geojson.geojson)}")
            ...
            if indicator_type == 'mobility':
                ...
                if 'trips' not in result:
                    ...
                    for i in range(100):
                        ...
                        for j in range(num_waypoints):
                            way_lon = start_lon + (end_lon - start_lon) * (j + 1) / (num_waypoints + 1)
                            way_lat = start_lat + (end_lat - start_lat) * (j + 1) / (num_waypoints + 1)
                        ...
                        trips.append({
                            ...
                            'mode': random.choice(['walk', 'bike', 'car', 'transit']),
                ...
                if 'transit' not in result:
                    ...
                    for i in range(10):
                        # Main axis
                        start_lon = bounds['west'] + random.random() * (bounds['east'] - bounds['west'])
                        start_lat = bounds['south'] + random.random() * (bounds['north'] - bounds['south'])
                        end_lon = bounds['west'] + random.random() * (bounds['east'] - bounds['west'])
                        end_lat = bounds['south'] + random.random() * (bounds['north'] - bounds['south'])
                ...
                if 'points' not in result:
                    ...
                    for i in range(1000):
                        ...
                        dx = (lon - (bounds['west'] + bounds['east']) / 2) / ((bounds['east'] - bounds['west']) / 2)
                        dy = (lat - (bounds['south'] + bounds['north']) / 2) / ((bounds['north'] - bounds['south']) / 2)
                        ...
                        points.append({
                            ...
                            'properties': {
                                'intensity': intensity,
                                'category': random.choice(['temperature', 'pollution', 'co2'])
                            }
                        })
                ...
                if 'boundaries' not in result:
                    ...
                    for i in range(5):
                        ...
                        west = bounds['west'] + random.random() * 0.7 * (bounds['east'] - bounds['west'])
                        south = bounds['south'] + random.random() * 0.7 * (bounds['north'] - bounds['south'])
                        width = random.random() * 0.3 * (bounds['east'] - bounds['west'])
                        height = random.random() * 0.3 * (bounds['north'] - bounds['south'])
                        ...
                        polygon_feature = {
                            ...
                            'properties': {
                                ...
                                'category': random.choice(['urban', 'suburban', 'park', 'industrial']),
                ...
                if 'buildings' not in result:
                    ...
                    for i in range(building_count):
                        ...
                        center_lon = bounds['west'] + random.random() * (bounds['east'] - bounds['west'])
                        center_lat = bounds['south'] + random.random() * (bounds['north'] - bounds['south'])
                        ...
                        height = float(random.random() * 100 + 10)  # Range from 10 to 110
                        
                        # Assign different colors based on building type
                        building_type = random.choice(['residential', 'commercial', 'industrial', 'mixed'])
            ...
            try:
                ...
                json_str = json.dumps(result, default=lambda obj: str(obj) if not isinstance(obj, (dict, list, str, int, float, bool, type(None))) else obj)
    ...
    def get_current_dashboard_data(self, request):
        ...
        response = JsonResponse({
            'data': dashboard_data.data, 
            'state': state.first().state_values
        })
        self._add_no_cache_headers(response)
        return response
