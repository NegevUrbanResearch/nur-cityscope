...
test("table switcher scripts explicitly export browser globals", () => {
  ...
  expect(popup.includes("window.TableSwitcherPopup = TableSwitcherPopup")).toBe(true);
});
...
test("viewport sync supports deferred map listener attachment", () => {
  ...
  expect(src.includes("window.attachViewportSyncListeners = attachViewportSyncListeners")).toBe(true);
});

test("layer name utils keeps direct global function contracts", () => {
  const src = read("frontend/src/shared/layer-name-utils.js");
  expect(src.includes("window.parseLayerNameWithGeometrySuffix = parseLayerNameWithGeometrySuffix")).toBe(true);
  expect(src.includes("window.normalizeLayerBaseName = normalizeLayerBaseName")).toBe(true);
});
