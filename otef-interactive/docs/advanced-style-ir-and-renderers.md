...
> **Note:** This document mixes active IR contracts with migration-era design notes.
> Current production rendering in `otef-interactive` is MapLibre-based for both GIS and projection.
> Any references to Leaflet/Protomaps/canvas frontend paths below should be read as historical background unless explicitly re-adopted.
...
- JS style engine → concrete renderers (MapLibre GIS + MapLibre projection)
...
- Use the existing style model through the current MapLibre layer managers and style bridge.
...
On the GIS advanced path, MapLibre layer materialization applies style-bridge output per source/layer while preserving class semantics from the IR. Tile origin and pattern alignment considerations still apply for hatch-like effects, and cache keys must stay stable for deterministic rendering.

### 3.3 Style resolution (advanced path)

The single place that resolves (feature, styleConfig, renderer) to a symbol IR is `AdvancedStyleEngine._resolveStyleSymbol`. For **uniqueValue** renderers, a Map from class value to symbol IR is built once per styleConfig and used for O(1) lookup; simple and defaultStyle resolution are unchanged.
...
- Know about specific map library internals directly.
...
- Map library context (MapLibre) for transforms.

**Responsibilities**:

- For advanced layers:
  - Use PMTiles primarily as **geometry + attribute backend**.
  - Keep styling logic in the shared bridge/engine path.
  - Materialize to MapLibre layers/sources in both GIS and projection runtimes.
- Ensure:
...
- Must coexist with:
  ...
  - GeoJSON-backed layers using the same MapLibre manager/bridge contracts.
...
- Keep using:
  - Current MapLibre style-bridge logic for PMTiles-backed and GeoJSON-backed simple layers.
