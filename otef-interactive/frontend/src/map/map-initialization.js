 ...
 */
import { buildMapOptions } from "./map-options.js";
...
const buildMapOptionsRef = buildMapOptions;
...
if (typeof window !== "undefined") {
  window.map = map;
  if (typeof window.attachViewportSyncListeners === "function") {
    window.attachViewportSyncListeners();
  }
}
...
function initializeMap(bounds) {
  ...
  if (typeof OTEFDataContext !== "undefined") {
    ...
    OTEFDataContext.init("otef")
      ...
      const loaderAPI = getMapLayerLoaderAPI();
      const mapDeps = {
        map,
        layerRegistry:
