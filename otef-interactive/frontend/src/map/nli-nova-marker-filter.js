export const NOVA_MARKER_FILTER = Object.freeze([
  "==",
  Object.freeze(["get", "location"]),
  "Nova",
]);

export function applyNovaMarkerFilter(map, narrativeId) {
  if (!map || typeof map.getStyle !== "function" || typeof map.getLayer !== "function" ||
    typeof map.setFilter !== "function") return 0;

  let style;
  try {
    style = map.getStyle();
  } catch (_) {
    return 0;
  }
  if (!Array.isArray(style?.layers)) return 0;

  const filter = narrativeId === "nova" ? NOVA_MARKER_FILTER : null;
  let updated = 0;
  for (const layer of style.layers) {
    if (!layer || layer.source !== "nli.people" || typeof layer.id !== "string") continue;
    let currentLayer;
    try {
      currentLayer = map.getLayer(layer.id);
    } catch (_) {
      continue;
    }
    if (!currentLayer) continue;
    map.setFilter(layer.id, filter);
    updated += 1;
  }
  return updated;
}
