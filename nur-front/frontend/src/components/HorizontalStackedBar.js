...
const HorizontalStackedBarChart = ({ data }) => {
  ...
  useEffect(() => {
    ...
    if (!data || !data.bars || !Array.isArray(data.bars) || data.bars.length === 0) {
      console.error('Invalid data structure for HorizontalStackedBarChart', data);
      return;
    }
    ...
    try {
      ...
      const validData = data.bars.every(bar => Array.isArray(bar.values) && bar.values.length > 0);
