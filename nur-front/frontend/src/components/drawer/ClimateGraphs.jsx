...
const useClimateData = (url = `${process.env.PUBLIC_URL}${config.api.endpoints.climateData}`,) => { 
...
const MetricLineChart = ({ title, data, xLabel, yLabel, seriesKeys }) => {
    ...
    return (
        <div style={{ width: "100%", height: 360, marginBottom: 24 }}>
            <ResponsiveContainer>
                <LineChart data={data} margin={{ top: 16, right: 24, bottom: 8, left: 12 }}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="Hour" label={{ value: xLabel, position: "insideBottom", offset: -2 }} />
            ...
            <div style={{ paddingInline: 4, marginTop: 6, fontWeight: 600 }}>{title}</div>
        </div>
    );
}